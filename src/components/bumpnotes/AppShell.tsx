...
export function AppShell({
  ...
  return (
    <>
      ...
      <div
        className={`hidden lg:grid ${right ? "lg:grid-cols-[240px_minmax(0,1fr)_320px]" : "lg:grid-cols-[240px_minmax(0,1fr)]"} lg:gap-8 lg:max-w-[1200px] lg:mx-auto lg:px-6 lg:py-8 lg:min-h-[100dvh]`}
      >
        <DesktopSidebar />
        <main className="min-w-0" data-clarity-mask="True">
          {children}
        </main>
  ...
  return (
    <div className="sticky top-0 z-30 w-full bg-[#E9DEF7] text-[#3F2C68] border-b border-[#D9C6F0] print:hidden">
      <div className="max-w-[1200px] mx-auto px-2.5 py-1 flex items-center gap-1.5 text-[10px] sm:text-[12px] leading-tight">
        ...
        <span className="opacity-80 min-w-0 flex-1">
          Have a look around — changes won't be saved.
        </span>
        <button
          onClick={exit}
...
function useNavItems() {
  ...
  return [
    ...
    { to: "/education" as const, label: t("nav.education"), Icon: BookOpen, matchExact: false },
  ...
  return (
    <div className="sticky top-8 space-y-4">
      <div className="surface-card p-5">
        ...
        <Link
          to="/pack"
          className="mt-4 inline-flex w-full justify-center py-2.5 rounded-full bg-primary text-primary-foreground text-sm font-semibold"
        >
          {t("sum.stepCreate")}
        </Link>
      </div>
  ...
  return (
    <div className="px-5 lg:px-1 pt-4 lg:pt-2 pb-3 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
      <div className="min-w-0">
        <h1 className="font-serif text-2xl lg:text-3xl font-semibold leading-tight break-words">
          {title}
        </h1>
