...
export function PregnancySummaryPreview({
  ...
  return (
    <div
      ...
      id="pack-print"
    >
      <h2 className="font-serif text-lg sm:text-xl font-semibold break-words">
        {t("sum.header.title")}
      </h2>
      ...
      <dl className="mt-4 grid grid-cols-[auto,1fr] sm:grid-cols-[auto,1fr,auto,1fr] gap-x-3 gap-y-1.5 text-xs">
        <dt className="text-ink-soft">{t("sum.field.name")}</dt>
        <dd className="font-medium break-words">{profile.userName}</dd>
        <dt className="text-ink-soft">{t("sum.field.baby")}</dt>
        <dd className="font-medium break-words">
          {profile.babyNickname?.trim() || t("baby.fallback")}
        </dd>
        <dt className="text-ink-soft">{t("sum.field.due")}</dt>
        <dd className="font-medium break-words">{formatUKDateLong(profile.dueDateISO)}</dd>
        <dt className="text-ink-soft">{t("sum.field.today")}</dt>
        <dd className="font-medium break-words">
          {formatGestation(gestationFromDueDate(profile.dueDateISO))}
        </dd>
  ...
  return (
    <div className="rounded-xl bg-blush-soft border border-border p-3 text-xs">
      ...
      {section.type === "symptoms" && (
        <SymptomList items={section.items} onHideItem={onHideItem} onReviewItem={onReviewItem} />
      )}
      {section.type === "feelings" && (
        <FeelingList items={section.items} onHideItem={onHideItem} onReviewItem={onReviewItem} />
      )}
    ...
    <ul className="space-y-1 text-ink">
      {items.map((item) => (
        <li key={item.feeling} className="flex items-start justify-between gap-2">
          <span>
            • {item.feeling}
            {item.days > 1 ? `, ${item.days} days this week` : ""}
          </span>
          <ItemActions
            itemKey={item.key}
            title={item.feeling}
