...
export function downloadSummaryPdf(opts: PdfOptions) {
  ...
  function text(
    s: string,
    opts2?: { size?: number; bold?: boolean; color?: [number, number, number]; gap?: number },
  ) {
  ...
  buildPregnancySummaryWeeks(opts.entries, {
    ...
    const estimatedWeekHeight =
      7 +
      week.sections.reduce((sum, section) => sum + 5 + sectionPdfLines(section).length * 4.5, 0);
    if (
      estimatedWeekHeight < PAGE_H - MARGIN_TOP - MARGIN_BOTTOM &&
      y + estimatedWeekHeight > PAGE_H - MARGIN_BOTTOM
    ) {
      footer(doc);
      doc.addPage();
      y = MARGIN_TOP;
  ...
  y += 4;
  rule();
  text(tFn("sum.foot").replace("{name}", opts.profile.userName), {
    size: 8,
    color: [120, 120, 130],
  });
...
function footer(doc: jsPDF) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      "Generated by BumpNotes — a personal pregnancy record. Not a medical record.",
      MARGIN_X,
      PAGE_H - 8,
    );
  }
