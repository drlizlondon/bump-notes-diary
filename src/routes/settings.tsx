            ...
            <div className="surface-card px-5 py-4 space-y-3">
              {tester ? (
                <>
                  ...
                  <p className="text-xs text-ink-soft">You're in a sandbox. Nothing here is real and nothing syncs to a cloud account.</p>
                  <button onClick={() => { store.clearAll(); exitTesterMode(); navigate({ to: "/welcome" }); }}
                    className="w-full py-2.5 rounded-full bg-white border border-border text-sm font-medium">Exit tester mode</button>
                </>
              ) : userId ? (
                <>
                  <div>
            ...
            <div className="surface-card divide-y divide-border overflow-hidden">
              ...
              >
                <span>
                  ...
                  <span className="block text-xs text-ink-soft mt-0.5">{entries.length} entries, profile, labour plan</span>
                </span>
                ...
                <div className="px-5 py-4 bg-blush-soft/50 text-xs space-y-2 max-h-72 overflow-auto">
                  <div>
                    ...
                    <p className="text-ink-soft break-words">
                      {profile ? `${profile.userName} · baby: ${profile.babyNickname || "—"} · due ${profile.dueDateISO}` : "No profile"}
                    </p>
                  </div>
                  <div>
                    ...
                    {entries.length > 50 && <p className="text-ink-soft italic mt-1">... and {entries.length - 50} more</p>}
                  </div>
                </div>
              )}
              <button onClick={exportData} className="w-full text-left px-5 py-4 text-sm font-medium">
              ...
              >
                {confirmWipe ? "Tap again to permanently delete your pregnancy record" : "Delete pregnancy record"}
              </button>
              {userId && (
                <button
                  ...
                  className="w-full text-left px-5 py-4 text-sm font-medium text-destructive"
                >
                  {confirmDeleteAccount ? "Tap again to permanently delete account & data" : "Permanently delete account and data"}
                </button>
              )}
            </div>
            <p className="text-[11px] text-ink-soft px-1 leading-relaxed">
              Deletion is immediate. Sign-in credentials may take up to 30 days to be removed from authentication logs by our hosting partner.
            </p>
          </section>
