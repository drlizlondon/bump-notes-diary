  ...
  return (
    <PublicShell>
      ...
      <section className="px-5 sm:px-8 pt-10 pb-12 max-w-[560px] mx-auto">
        <p className="text-xs uppercase tracking-widest text-primary font-semibold">Get in contact</p>
        <h1 className="font-serif text-3xl sm:text-4xl font-semibold mt-2">We'd love to hear from you</h1>
        <p className="text-ink-soft mt-3 leading-relaxed">
          Whether you have a question, a suggestion or want to share your story — drop us a note.
          For private feedback inside the app there is also a small button in the corner once you're using BumpNotes.
        </p>

        {sent ? (
          <div className="surface-card p-5 mt-6 blush-bg">
            <p className="font-serif text-lg font-semibold">Thank you 💛</p>
            <p className="text-sm text-ink-soft mt-1">Your message has been received. We read everything personally.</p>
          </div>
        ) : (
          <form onSubmit={send} className="surface-card p-5 mt-6 space-y-3">
            ...
            <button disabled={busy} className="w-full py-3 rounded-full bg-primary text-primary-foreground text-sm font-semibold disabled:opacity-50">
