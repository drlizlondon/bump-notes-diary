...
- Labour/contraction-timer removal (PLAN §10 Phase 1, commits `5f3650f`–`952eeb3`) — deliberately removed the fastest path across the device boundary ("when should I go in?" is triage; ARCH §10). **Extended and made permanent 12 Sep 2026 (founder ruling):** labour is excluded from the Azure V2 launch schema/app entirely (never added to migrations 001/002 or the V2 domain types) and the legacy V1 data model is archived, not deleted — `docs/product/LABOUR-ARCHIVE.md` records what "on the system" means layer by layer and the re-enable recipe. Any labour data in an old user's blob is preserved unmapped in `bumpnotes_state_archive` (migration 002) rather than crossing into any active V2 table — the retention/disposal of that archive follows the same schedule as the rest of the legacy blob archive (§3 Retention, below).
...
Existing evidence:
- **Residency:** Azure resources provisioned 8 Sep 2026 — PostgreSQL, Storage, Key Vault, App Insights all **UK South**; the App Service (`bumpnotes-api`) is **UK West** (UK South had no App Service compute quota on the trial subscription — still UK data residency) and the Entra External ID tenant's data location is **UK/EU** (AZURE §1, §2.1 record). Supabase (current, interim) is EU-hosted; residency improves at cutover.
...
- Migration 001 (AZURE 2.3, `azure/migrations/20260712120000_v2_core_identity_and_about_me.sql`): identity separated from the identity provider (`users` internal UUID + nullable `external_identity_id`/`supabase_user_id`), case-insensitive unique email (Phase I linking precondition), per-region `health_identifier` + label (never a hardcoded NHS-number concept), CASCADE deletion from `users` (erasure path), no RLS by design (API-level authorization per AZURE §2 — record in pen-test scope alongside the 8A.3 note). ~~Authored and locally verified only (12 Jul 2026, throwaway PG 16); no Azure environment exists yet — not applied, not provisioned.~~ **UPDATE 8 Sep 2026: applied to the real Azure `bumpnotes` database (PG 18.6, UK South) via the checksummed 2.2 runner — 2 applied/0 pending, idempotent; schema inspected, 0 rows.**
- Migration 002 (AZURE 2.4, `azure/migrations/20260717120000_v2_entries_attachments_summaries_audit.sql`): adds `entries`/`attachments`/`summaries`/`audit_events`/`bumpnotes_state_archive`. Summaries immutability trigger verified to reject frozen-field edits and permit `pdf_path`/`shared`; `audit_events` verified append-only (UPDATE and DELETE both rejected by trigger); `bumpnotes_state_archive` keyed by `supabase_user_id` (no FK to `users` — carries accounts that may never authenticate on Azure). ~~Authored and locally verified only (17 Jul 2026, throwaway PG 16); no Azure environment exists yet — not applied, not provisioned.~~ **UPDATE 8 Sep 2026: applied to the real Azure `bumpnotes` database (PG 18.6, UK South); the summaries immutability trigger (rejects frozen-field edits, permits `pdf_path`/`shared`) and `audit_events` append-only trigger (rejects UPDATE and DELETE) were both re-verified green against real Azure in a throwaway `bumpnotes_verify` DB, since dropped.**
- API auth middleware (AZURE 2.6, `src/lib/azure/api-auth-middleware.ts`): accepts either a Supabase JWT (bridge window, verified via the existing Supabase client) or an Entra access token (verified via `jose` against a remote JWKS with issuer + audience enforcement), then resolves/creates the internal `users` row by source-specific external id only — no email-based linking (that's Phase I.2, deliberately deferred). No server function is wired to this middleware yet (awaits the 3.2 repository layer and 2.1 provisioning for real Entra/PG config). **Authored and locally verified only (17 Jul 2026): idempotent user resolution against a throwaway PG 16 database; `verifyEntraToken` proven to accept a correctly-signed token and reject wrong-audience and wrong-signature tokens using a locally generated RSA keypair (no live Entra tenant involved).** **UPDATE 8 Sep 2026: the real Entra External ID external tenant now exists (`BumpNotes`, tenant `23f549b5-…`, UK/EU) with the API app (`email` optional claim on the access token) + SPA app (PKCE, admin-consented `access_as_user`) + `SignUpSignIn` user flow, and a separate workforce-tenant admin registration (distinct issuer + audience — a customer token cannot reach admin). The three middleware env values (`AZURE_ENTRA_ISSUER`/`_JWKS_URI`/`_AUDIENCE`) are recorded in AZURE §2.1; issuer/JWKS were read verbatim from the tenant OIDC metadata. Wiring the middleware into routes + App Service config is 2.5/3.2. Open: confirm the access-token `aud` form against a real token at I.1.**
- Blob helper endpoints (AZURE 2.7, `src/lib/azure/blob-helpers.ts`): upload/SAS-issuance/delete all ownership-checked against `attachments.user_id`, 10 MB cap enforced before any storage call, SAS URLs read-only and 5-minute TTL. **Known gap, tracked open in the AZURE plan checklist (not ticked):** SAS signing currently uses a shared account key (`StorageSharedKeyCredential`) rather than managed identity — this contradicts the AZURE §1.2 "no account keys" non-negotiable and must not be wired into any live route or deployed config until it's swapped for `@azure/identity` + user-delegation SAS, verified against a real App Service managed identity. **Authored and locally verified only (17 Jul 2026):** full upload → SAS-fetch → delete cycle exercised against a local Azurite emulator + throwaway PG 16, including forbidding upload/SAS-issuance/delete for a non-owning user and rejecting an over-cap upload before any storage call.
