...
- **STATUS 8 Sep 2026 — attempted, BLOCKED on bundling; reverted to keep the deploy green.** `applicationinsights@3.16` cannot be bundled into the nitro/rollup node-server build: it pulls in `@microsoft/applicationinsights-web-snippet`, whose export shape breaks rollup chunk render (`Cannot read properties of null (reading 'getVariableForExportName')`). `tsc --noEmit` was clean — only rollup bundling fails. Tried and did NOT fix it: nitro `externals: { external: ["applicationinsights"] }` (it traced into `.output/server/node_modules` but rollup still followed the app-level import), and `import type` + dynamic `import("applicationinsights")`. Reverted (uninstalled the dep, restored `server.ts`/`vite.config.ts`), build green again.
  - **Known-good design to reuse (this session's code, ~80 lines):** a `src/lib/azure/telemetry.ts` using a **standalone `TelemetryClient`, manual-only** (never call `setup()/start()` → no auto-instrumentation → no PII from auto-collection). Emit only `method`, the **scrubbed path** (`new URL(url).pathname`, query/fragment dropped), status, duration; plus `trackException(err, {path})`. No-op when `APPLICATIONINSIGHTS_CONNECTION_STRING` is unset (local/preview). Wire into `src/server.ts`'s fetch handler: time each **non-health** request, `trackRequest` after; `trackServerException` + a 500 `trackRequest` in the catch. This is PII-safe by construction (path-only, no bodies/headers/query).
  - **Fix directions (do deliberately, not at speed):** (a) also externalize `@microsoft/applicationinsights-web-snippet` and confirm the exact nitro externals API through the `@lovable.dev/vite-tanstack-config` wrapper; (b) evaluate `@azure/monitor-opentelemetry` / the OTel exporter, which may bundle cleanly; (c) a small rollup plugin to stub the web-snippet on the server build; (d) last resort: App Service platform auto-instrumentation — **rejected unless PII can be fully controlled**, since it captures request data; the manual-SDK path above is preferred precisely for the PII guarantee. Set `APPLICATIONINSIGHTS_CONNECTION_STRING` (from `bumpnotes-insight`) as the final app setting once bundling is solved.
