...
## PROVEN END TO END — 9 Sep 2026 — the B login works (commit `e96d061`)
Tenant enabled (native auth + public client, step 1 done). Ran the full flow locally against the real CIAM tenant, driving BumpNotes' own branded form: **sign-up → email verification → signed in**, **forgot-password → emailed code → new password → signed in**, and **email+password sign-in → signed in**. The `/entra` page reached the ✅ signed-in panel and attached the API access token to the Azure call (local profile load stops only at `AZURE_ENTRA_JWKS_URI`, which lives on the deployed instance — so it confirms the token flows). Steps 1–3 are effectively done except running it once on the deployed instance to see the Postgres write; step 4 (Phase 3 cutover onto the live `/signin`) is unchanged.

### The load-bearing finding for the playbook: CIAM native auth omits `client_info`
Native auth's token endpoint (`/oauth2/v2.0/token`, `grant_type=password` with a continuation_token) returns a **valid** access/refresh/id token but **no `client_info`** field, and its id_token lacks `oid`. MSAL browser needs one or the other to build the account, so it throws **`client_info_missing`** and never caches the session — even though the token is good. Sending `client_info=1` on the request does **not** make CIAM return it (verified). **Fix (in `src/server.ts`): the same-origin proxy synthesizes `client_info` from the token's own `oid`+`tid` (`base64url({uid: oid, utid: tid})`) and injects it into the response.** This is now part of the reusable proxy — every future product on native auth needs it until Microsoft returns `client_info` natively. Watch for regressions if MSAL starts returning it (the inject is guarded to skip when `client_info` is already present).

### Other observed CIAM behaviours (handled in the UI)
- **Banned-password list** — a weak/common new password is rejected at `resetpassword/v1.0/submit` with `AADSTS399249 password_banned`; the form keeps you on the new-password step to retype without a fresh code.
- **Password-required accounts reject the OTP path** — "Email me a code" returns "password required" for a password account; sign in with the password (or reset it).
- **Security note:** never log the token-exchange request body — the sign-in grant carries the password in it.

## FULL ROUND-TRIP PROVEN ON THE DEPLOYED AZURE INSTANCE — 9 Sep 2026
Deployed to `bumpnotes-api` (App Service, UK West) via the gated GitHub Actions workflow (`VITE_ENTRA_NATIVE=true`). On the live `/entra`: signed in through BumpNotes' own form → the API validated the Entra token (JWKS/issuer/audience env present) → **wrote a profile row to UK West Postgres and read it back** (`displayName: "Liz London"`, stable `userId` derived from the Entra identity). The entire pure-UK stack — identity, API, database — works end to end, driven by the native-auth login. Live `/signin` remains on Supabase (Phase 3 cutover unchanged).

### The database-connection rabbit hole (hours lost — read this before the next product)
The profile call failed for a long time with `getaddrinfo ENOTFOUND base`. Root-cause chain, in order of what we ruled out:
- **`base` is not a hostname anyone set — it is `pg`'s fallback.** `pg-connection-string` does `new URL(connString, 'postgres://base')`. When the connection string is **not a valid absolute URL**, `new URL` resolves it against that base and the host becomes literally **`base`**. So "ENOTFOUND base" always means **"AZURE_PG_URL is not a valid `postgres://` URL at runtime"** — never a real host typo. (Verified: a `/` or `< >` in the password, or the literal `@Microsoft.KeyVault(...)` reference string, all trigger it.)
- **What actually reached the app was wrong repeatedly:** first the Key Vault secret held the literal template with `<your-db-password>`/`<database>` placeholders (angle brackets → invalid URL → `base`); creating new secret versions did nothing because **App Service was not serving the resolved Key Vault reference** even after a restart *and* an app-settings change (which should force re-resolution). Every prerequisite was correct — reference `@Microsoft.KeyVault(VaultName=bumpnotes-kv;SecretName=AzurePgUrl)` (unpinned → latest), the app's managed identity **has** "Key Vault Secrets User" on the vault (RBAC), vault networking open to all — yet the app kept getting `base`. This is an unresolved **App Service Key Vault-reference caching** issue.
- **What unblocked it:** setting `AZURE_PG_URL` to the **plaintext** connection string (`postgresql://drlizlondon:<pw>@bumpnotes.postgres.database.azure.com:5432/bumpnotes?sslmode=require`) — bypassing the stuck KV indirection — and it worked immediately. **DB details for the record:** host `bumpnotes.postgres.database.azure.com`, user `drlizlondon`, database **`bumpnotes`** (not `postgres`), `sslmode=require`.

**Follow-ups (pre-launch):**
1. **Security:** `AZURE_PG_URL` is currently plaintext in App Service config (a health-app no-no). Move it back to the Key Vault reference once the caching issue is understood (candidate: let the KV cache expire ~24h then re-point, or delete+re-add the reference, or switch the app to a user-assigned identity for KV). Re-verify `/entra` after.
2. Remove the throwaway `KV_REFRESH` app setting added during debugging.
3. DB password was rotated during this session (PG server → Reset password). Choose a password with **no URL-special characters** (`/ : ? # % & = + @` or spaces) so the connection string always parses — `:` and `@` happen to survive `new URL`, but `/` does not.
